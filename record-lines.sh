...
# 이전 기록이 있는지 확인
previous_record=$(grep "^$date|" record-lines.txt)
...
if [ -n "$previous_record" ]; then
  ...
  grep -v "^$date|" record-lines.txt > temp.txt
  echo "$date|$lines" >> temp.txt
  mv temp.txt record-lines.txt
else
  # 이전 기록이 없는 경우에는 새로운 줄로 추가
  echo "$date|$lines" >> record-lines.txt
fi